...
`$research-orchestrator`，以及 170 多个只可显式调用的科研工作单元，覆盖
12 个科研步骤、8.5 写作原创性与可追溯性闸门，以及跨项目 Timeline OS。
...
报告；个人本地 Portfolio 只保存项目路径和脱敏进度摘要。它不会自动读取原始敏感数据、杜撰证据、规避 AI 检测，或实际投递稿件。
