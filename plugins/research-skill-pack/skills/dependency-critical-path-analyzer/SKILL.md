...
- **本单元：**`dependency-critical-path-analyzer`，属于 Research Timeline & Milestone OS。只在它自己的输入已出现、需要产生 `critical_path_report` 或需要检查 `dependencies-present` 时调用。
- **Catalog 输入：**`execution_timeline`。先记录每项是用户提供、已有 artifact、可定位本地资料、未知，还是仅为模型推断。
- **Catalog 输出：**`critical_path_report`。输出是受限建议、草稿或版本化记录，不自动改变研究状态。
- **本单元闸门：**`dependencies-present`。闸门不满足时不以“合理猜测”替代缺失信息。
- **本单元安全约束：**`flag-unknown-dependencies`。这些限制与项目的证据字段、版本规则同等优先。

## 前置核对

1. 读取 `<project>/.research/project.yaml`、关联 artifact 的版本与状态；已确认内容复用，不重复要求研究者提供。
2. 对 `execution_timeline` 建立输入清单：来源定位、可用范围、`provenance`、`verification_status` 与缺失值必须分开。
3. 检查 `dependency-critical-path-analyzer` 是否与当前研究状态相容，并逐项核对 `dependencies-present`；冲突或未知项先进入待解决清单。
4. 明确本次处理的最小对象与输出边界：只服务 `critical_path_report`，不顺带替代上游决策、真实数据处理或外部操作。
...
4. 围绕 `critical_path_report` 形成可追溯草稿：每个关键判断注明输入 ID、依据、限制和需要人工确认的内容。
5. 将时间冲突、单点依赖和剩余缓冲单列，给出范围收缩或重排方案而非静默改期。
6. 只把待确认的基线、计划或重排建议写成新版本；实际进度应追加 check-in，不能改写历史。

## 产物与记录

对 `critical_path_report` 创建新版本，而不是覆盖旧记录。最小工作草图如下；字段值必须来自本次可定位输入：

```yaml
critical_path_report:
  artifact_id: <dependency-critical-path-analyzer-sequence>
  status: draft
  ...
  gate_result:
    dependencies-present: pass | unmet | not_applicable
...
- 若 `critical_path_report` 会影响下游内容，写明 `depends_on`、`supersedes` 与受影响项；只生成建议时标记 `advisory_only`。
...
|---|---|---|---|
| `ready` | 输入可定位，`dependencies-present` 均满足，且输出边界已说明 | 写入版本化 `critical_path_report` 草稿/记录 | 交给状态机允许的下游 Skill；是否确认仍由用户决定 |
...
- `dependency-critical-path-analyzer` 只基于可定位的本地资料工作；不能联网补检索、抓取全文、伪造来源、页码、数据、结果或人工核验。
- 原始敏感数据默认不读入模型；如涉及数据，只登记路径、哈希、权限和经批准的最小脱敏摘要。
- 不登录外部系统、不发送邮件、不代表用户同意声明、不提交稿件；任何此类动作都在本 Skill 范围外。
- 安全限制 `flag-unknown-dependencies` 发生冲突时，以更严格边界为准，并说明为什么不能继续。
...
- 若 `execution_timeline` 缺失或无法定位：不补造内容，`decision` 为 `blocked`，并把最小补充要求写入 `critical_path_report`。
- 若 `execution_timeline` 已定位但 `dependencies-present` 尚未满足：产物只保留为 `draft` / `requires_confirmation`，不能将其作为下游已确认依据。
- 只有输入可追溯、闸门结果可说明且边界已写明时，才把 `critical_path_report` 交给下游 Skill 继续处理。

## 本单元完成前自检

- `dependency-critical-path-analyzer` 是否仅处理已声明的输入，并让 `critical_path_report` 可以回到具体的输入定位？
- `dependencies-present` 是否有明确的 `pass`、`requires_confirmation` 或 `blocked` 结果，而不是被隐含跳过？
