...
description: Simulate a domain review of topic, theory, contribution, and literature
  coverage using project evidence. Explicit only.
...
- 产物：`domain_review`。
- 能力说明：Simulate a domain review of topic, theory, contribution, and literature coverage using project evidence. Explicit only.
...
- **本单元：**`domain-reviewer`，属于 Manuscript, Audit & Reviewer。只在它自己的输入已出现、需要产生 `domain_review` 或需要检查 `scope-known` 时调用。
- **Catalog 输入：**`manuscript`。先记录每项是用户提供、已有 artifact、可定位本地资料、未知，还是仅为模型推断。
- **Catalog 输出：**`domain_review`。输出是受限建议、草稿或版本化记录，不自动改变研究状态。
- **本单元闸门：**`scope-known`。闸门不满足时不以“合理猜测”替代缺失信息。
...
2. 对 `manuscript` 建立输入清单：来源定位、可用范围、`provenance`、`verification_status` 与缺失值必须分开。
3. 检查 `domain-reviewer` 是否与当前研究状态相容，并逐项核对 `scope-known`；冲突或未知项先进入待解决清单。
4. 明确本次处理的最小对象与输出边界：只服务 `domain_review`，不顺带替代上游决策、真实数据处理或外部操作。
...
- 若 `domain_review` 会影响下游内容，写明 `depends_on`、`supersedes` 与受影响项；只生成建议时标记 `advisory_only`。
...
|---|---|---|---|
| `ready` | 输入可定位，`scope-known` 均满足，且输出边界已说明 | 写入版本化 `domain_review` 草稿/记录 | 交给状态机允许的下游 Skill；是否确认仍由用户决定 |
...
- `domain-reviewer` 只基于可定位的本地资料工作；不能联网补检索、抓取全文、伪造来源、页码、数据、结果或人工核验。
...
- 若 `manuscript` 缺失或无法定位：不补造内容，`decision` 为 `blocked`，并把最小补充要求写入 `domain_review`。
- 若 `manuscript` 已定位但 `scope-known` 尚未满足：产物只保留为 `draft` / `requires_confirmation`，不能将其作为下游已确认依据。
- 只有输入可追溯、闸门结果可说明且边界已写明时，才把 `domain_review` 交给下游 Skill 继续处理。

## 本单元完成前自检

- `domain-reviewer` 是否仅处理已声明的输入，并让 `domain_review` 可以回到具体的输入定位？
- `scope-known` 是否有明确的 `pass`、`requires_confirmation` 或 `blocked` 结果，而不是被隐含跳过？
