...
description: 把已核验的 Paper、变量、方法、发现、局限和 Future Work 组织为研究矩阵。 用于 Research Skill Pack 的细粒度科研工作单元；仅在用户显式调用时使用。
workflow_depth: operational
---

# 文献矩阵构建

把已核验的 Paper、变量、方法、发现、局限和 Future Work 组织为研究矩阵。

## 前置与输入

- 读取[项目契约](../../shared/project-contract.md)与[状态机](../../shared/state-machine.md)，并检查项目状态、已有 artifact 和依赖。
- 输入：结构化解析与 Claim Library。

## 执行与产物

- 只处理满足前置条件的材料；缺少依赖时说明缺口并路由至对应上游 Skill。
- 输出：可追溯 Literature Matrix。。按项目契约创建新版本化 artifact，包含依赖、变更理由和三项证据字段；绝不静默覆盖。
...
- **本单元：**`literature-matrix-builder`，属于 Literature, Gap & Review Board。只在它自己的输入已出现、需要产生 `literature_matrix` 或需要检查 `field-locations` 时调用。
- **Catalog 输入：**`structured_paper_records`。先记录每项是用户提供、已有 artifact、可定位本地资料、未知，还是仅为模型推断。
- **Catalog 输出：**`literature_matrix`。输出是受限建议、草稿或版本化记录，不自动改变研究状态。
- **本单元闸门：**`field-locations`。闸门不满足时不以“合理猜测”替代缺失信息。
- **本单元安全约束：**`missing-fields-disclosed`。这些限制与项目的证据字段、版本规则同等优先。
...
3. 检查 `literature-matrix-builder` 是否与当前研究状态相容，并逐项核对 `field-locations`；冲突或未知项先进入待解决清单。
4. 明确本次处理的最小对象与输出边界：只服务 `literature_matrix`，不顺带替代上游决策、真实数据处理或外部操作。
...
4. 围绕 `literature_matrix` 形成可追溯草稿：每个关键判断注明输入 ID、依据、限制和需要人工确认的内容。
5. 维护支持、冲突、替代和未决关系；不能因多数资料同向就抹掉反例或范围差异。
6. 把空白、问题或评审建议写成带证据边界的草稿，缺少可核对材料时回到导入、阅读或核验。

## 产物与记录

对 `literature_matrix` 创建新版本，而不是覆盖旧记录。最小工作草图如下；字段值必须来自本次可定位输入：

```yaml
literature_matrix:
  artifact_id: <literature-matrix-builder-sequence>
  status: draft
  ...
  gate_result:
    field-locations: pass | unmet | not_applicable
...
- 若 `literature_matrix` 会影响下游内容，写明 `depends_on`、`supersedes` 与受影响项；只生成建议时标记 `advisory_only`。
...
|---|---|---|---|
| `ready` | 输入可定位，`field-locations` 均满足，且输出边界已说明 | 写入版本化 `literature_matrix` 草稿/记录 | 交给状态机允许的下游 Skill；是否确认仍由用户决定 |
...
- `literature-matrix-builder` 只基于可定位的本地资料工作；不能联网补检索、抓取全文、伪造来源、页码、数据、结果或人工核验。
- 原始敏感数据默认不读入模型；如涉及数据，只登记路径、哈希、权限和经批准的最小脱敏摘要。
- 不登录外部系统、不发送邮件、不代表用户同意声明、不提交稿件；任何此类动作都在本 Skill 范围外。
- 安全限制 `missing-fields-disclosed` 发生冲突时，以更严格边界为准，并说明为什么不能继续。
...
- 若 `structured_paper_records` 缺失或无法定位：不补造内容，`decision` 为 `blocked`，并把最小补充要求写入 `literature_matrix`。
- 若 `structured_paper_records` 已定位但 `field-locations` 尚未满足：产物只保留为 `draft` / `requires_confirmation`，不能将其作为下游已确认依据。
- 只有输入可追溯、闸门结果可说明且边界已写明时，才把 `literature_matrix` 交给下游 Skill 继续处理。

## 本单元完成前自检

- `literature-matrix-builder` 是否仅处理已声明的输入，并让 `literature_matrix` 可以回到具体的输入定位？
- `field-locations` 是否有明确的 `pass`、`requires_confirmation` 或 `blocked` 结果，而不是被隐含跳过？
