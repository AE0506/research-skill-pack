...
description: 汇总已通过的设计 artifact 形成可执行且可审计的 Research Protocol 草案。 用于 Research Skill Pack 的细粒度科研工作单元；仅在用户显式调用时使用。
workflow_depth: operational
---

# 研究方案构建

汇总已通过的设计 artifact 形成可执行且可审计的 Research Protocol 草案。

## 前置与输入

- 读取[项目契约](../../shared/project-contract.md)与[状态机](../../shared/state-machine.md)，并检查项目状态、已有 artifact 和依赖。
- 输入：问题、理论模型、假设、变量、测量、样本、控制与分析计划。

## 执行与产物

- 只处理满足前置条件的材料；缺少依赖时说明缺口并路由至对应上游 Skill。
- 输出：Research Protocol Draft。。按项目契约创建新版本化 artifact，包含依赖、变更理由和三项证据字段；绝不静默覆盖。
- 对结论给出可核对的来源定位、限制与待人工确认项；需要用户确认的决策始终保持 draft。

## 安全边界

- 用户确认前不得推进 design_ready；缺少关键 artifact 时阻断。
...
- **本单元：**`research-protocol-builder`，属于 Research Design, Data & Evidence。只在它自己的输入已出现、需要产生 `research_protocol` 或需要检查 `all-design-components` 时调用。
- **Catalog 输入：**`design_artifacts`。先记录每项是用户提供、已有 artifact、可定位本地资料、未知，还是仅为模型推断。
- **Catalog 输出：**`research_protocol`。输出是受限建议、草稿或版本化记录，不自动改变研究状态。
- **本单元闸门：**`all-design-components`。闸门不满足时不以“合理猜测”替代缺失信息。
- **本单元安全约束：**`user-confirmation-needed`。这些限制与项目的证据字段、版本规则同等优先。

## 前置核对

1. 读取 `<project>/.research/project.yaml`、关联 artifact 的版本与状态；已确认内容复用，不重复要求研究者提供。
2. 对 `design_artifacts` 建立输入清单：来源定位、可用范围、`provenance`、`verification_status` 与缺失值必须分开。
3. 检查 `research-protocol-builder` 是否与当前研究状态相容，并逐项核对 `all-design-components`；冲突或未知项先进入待解决清单。
...
```yaml
research_protocol:
  artifact_id: <research-protocol-builder-sequence>
  status: draft
  depends_on: [<confirmed-or-versioned-input-artifact-id>]
  input_trace:
    design_artifacts: supplied | artifact_linked | missing | needs_confirmation
  gate_result:
    all-design-components: pass | unmet | not_applicable
...
|---|---|---|---|
| `ready` | 输入可定位，`all-design-components` 均满足，且输出边界已说明 | 写入版本化 `research_protocol` 草稿/记录 | 交给状态机允许的下游 Skill；是否确认仍由用户决定 |
...
- `research-protocol-builder` 只基于可定位的本地资料工作；不能联网补检索、抓取全文、伪造来源、页码、数据、结果或人工核验。
- 原始敏感数据默认不读入模型；如涉及数据，只登记路径、哈希、权限和经批准的最小脱敏摘要。
- 不登录外部系统、不发送邮件、不代表用户同意声明、不提交稿件；任何此类动作都在本 Skill 范围外。
- 安全限制 `user-confirmation-needed` 发生冲突时，以更严格边界为准，并说明为什么不能继续。
...
- 若 `design_artifacts` 缺失或无法定位：不补造内容，`decision` 为 `blocked`，并把最小补充要求写入 `research_protocol`。
- 若 `design_artifacts` 已定位但 `all-design-components` 尚未满足：产物只保留为 `draft` / `requires_confirmation`，不能将其作为下游已确认依据。
- 只有输入可追溯、闸门结果可说明且边界已写明时，才把 `research_protocol` 交给下游 Skill 继续处理。

## 本单元完成前自检

- `research-protocol-builder` 是否仅处理已声明的输入，并让 `research_protocol` 可以回到具体的输入定位？
- `all-design-components` 是否有明确的 `pass`、`requires_confirmation` 或 `blocked` 结果，而不是被隐含跳过？
- 本次记录是否保留了证据限制、未决项与下游影响，且没有把草稿或模型推断升级为事实？

## 规范化写入

- **策略：**`canonical_writer`。本 Skill 的唯一 canonical 权限来自 [规范化写入策略表](../../shared/canonical-mutation-policy-v0.2.yaml)：artifact 为 `research_protocol`；状态迁移为 `board_decided → design_ready`。不按名称猜测或扩大权限。
- **开始前：**先调用 `inspect_research_project`，读取 `mutation_revision`、现有 artifact、状态闸门与未收敛收据；若状态为 `blocked`，先说明恢复条件。
- **预检：**构造一次变更请求（`request_id`、`expected_mutation_revision`、`origin_skill_id: research-protocol-builder`、artifact 和/或状态迁移原因），调用 `validate_canonical_change`。预检失败时不写 `.research/`。
