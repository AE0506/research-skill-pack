...
- **本单元：**`research-question-generator`，属于 Literature, Gap & Review Board。只在它自己的输入已出现、需要产生 `research_question_candidates` 或需要检查 `context-confirmed` 时调用。
- **Catalog 输入：**`gap_cards`、`normalized_constraints`。先记录每项是用户提供、已有 artifact、可定位本地资料、未知，还是仅为模型推断。
- **Catalog 输出：**`research_question_candidates`。输出是受限建议、草稿或版本化记录，不自动改变研究状态。
...
2. 对 `gap_cards`、`normalized_constraints` 建立输入清单：来源定位、可用范围、`provenance`、`verification_status` 与缺失值必须分开。
3. 检查 `research-question-generator` 是否与当前研究状态相容，并逐项核对 `context-confirmed`；冲突或未知项先进入待解决清单。
4. 明确本次处理的最小对象与输出边界：只服务 `research_question_candidates`，不顺带替代上游决策、真实数据处理或外部操作。
...
4. 围绕 `research_question_candidates` 形成可追溯草稿：每个关键判断注明输入 ID、依据、限制和需要人工确认的内容。
5. 维护支持、冲突、替代和未决关系；不能因多数资料同向就抹掉反例或范围差异。
6. 把空白、问题或评审建议写成带证据边界的草稿，缺少可核对材料时回到导入、阅读或核验。

## 产物与记录

对 `research_question_candidates` 创建新版本，而不是覆盖旧记录。最小工作草图如下；字段值必须来自本次可定位输入：

```yaml
research_question_candidates:
  artifact_id: <research-question-generator-sequence>
  status: draft
...
- 若 `research_question_candidates` 会影响下游内容，写明 `depends_on`、`supersedes` 与受影响项；只生成建议时标记 `advisory_only`。
...
|---|---|---|---|
| `ready` | 输入可定位，`context-confirmed` 均满足，且输出边界已说明 | 写入版本化 `research_question_candidates` 草稿/记录 | 交给状态机允许的下游 Skill；是否确认仍由用户决定 |
...
- `research-question-generator` 只基于可定位的本地资料工作；不能联网补检索、抓取全文、伪造来源、页码、数据、结果或人工核验。
...
- 若 `gap_cards` 缺失或无法定位：不补造内容，`decision` 为 `blocked`，并把最小补充要求写入 `research_question_candidates`。
- 若 `gap_cards` 已定位但 `context-confirmed` 尚未满足：产物只保留为 `draft` / `requires_confirmation`，不能将其作为下游已确认依据。
- 只有输入可追溯、闸门结果可说明且边界已写明时，才把 `research_question_candidates` 交给下游 Skill 继续处理。

## 本单元完成前自检

- `research-question-generator` 是否仅处理已声明的输入，并让 `research_question_candidates` 可以回到具体的输入定位？
