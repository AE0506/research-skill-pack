---
name: research-question-operationalizer
description: 把已确认研究问题拆成对象、关系、情境、可观察结果和证据需求。 用于 Research Skill Pack 的细粒度科研工作单元；仅在用户显式调用时使用。
workflow_depth: operational
---

# 研究问题操作化

把已确认研究问题拆成对象、关系、情境、可观察结果和证据需求。

## 前置与输入

- 读取[项目契约](../../shared/project-contract.md)与[状态机](../../shared/state-machine.md)，并检查项目状态、已有 artifact 和依赖。
- 输入：确认的研究问题和 Gap Card。

## 执行与产物

- 只处理满足前置条件的材料；缺少依赖时说明缺口并路由至对应上游 Skill。
- 输出：Research Question Operationalization。。按项目契约创建新版本化 artifact，包含依赖、变更理由和三项证据字段；绝不静默覆盖。
...
- **本单元：**`research-question-operationalizer`，属于 Research Design, Data & Evidence。只在它自己的输入已出现、需要产生 `operationalization_record` 或需要检查 `observable-terms` 时调用。
- **Catalog 输入：**`research_question`。先记录每项是用户提供、已有 artifact、可定位本地资料、未知，还是仅为模型推断。
- **Catalog 输出：**`operationalization_record`。输出是受限建议、草稿或版本化记录，不自动改变研究状态。
- **本单元闸门：**`observable-terms`。闸门不满足时不以“合理猜测”替代缺失信息。
- **本单元安全约束：**`no-measurement-invention`。这些限制与项目的证据字段、版本规则同等优先。

## 前置核对

1. 读取 `<project>/.research/project.yaml`、关联 artifact 的版本与状态；已确认内容复用，不重复要求研究者提供。
2. 对 `research_question` 建立输入清单：来源定位、可用范围、`provenance`、`verification_status` 与缺失值必须分开。
3. 检查 `research-question-operationalizer` 是否与当前研究状态相容，并逐项核对 `observable-terms`；冲突或未知项先进入待解决清单。
4. 明确本次处理的最小对象与输出边界：只服务 `operationalization_record`，不顺带替代上游决策、真实数据处理或外部操作。
...
4. 围绕 `operationalization_record` 形成可追溯草稿：每个关键判断注明输入 ID、依据、限制和需要人工确认的内容。
5. 把数据访问、样本、测量、方法、统计假设与伦理限制逐项标为已证实、待确认或不可行。
6. 写入可审计的计划、登记或证据包；无真实获取路径、分析谱系或人工核验时不得升级为结果。

## 产物与记录

对 `operationalization_record` 创建新版本，而不是覆盖旧记录。最小工作草图如下；字段值必须来自本次可定位输入：

```yaml
operationalization_record:
  artifact_id: <research-question-operationalizer-sequence>
  status: draft
  depends_on: [<confirmed-or-versioned-input-artifact-id>]
  input_trace:
    research_question: supplied | artifact_linked | missing | needs_confirmation
  gate_result:
    observable-terms: pass | unmet | not_applicable
...
- 若 `operationalization_record` 会影响下游内容，写明 `depends_on`、`supersedes` 与受影响项；只生成建议时标记 `advisory_only`。
...
|---|---|---|---|
| `ready` | 输入可定位，`observable-terms` 均满足，且输出边界已说明 | 写入版本化 `operationalization_record` 草稿/记录 | 交给状态机允许的下游 Skill；是否确认仍由用户决定 |
...
- `research-question-operationalizer` 只基于可定位的本地资料工作；不能联网补检索、抓取全文、伪造来源、页码、数据、结果或人工核验。
- 原始敏感数据默认不读入模型；如涉及数据，只登记路径、哈希、权限和经批准的最小脱敏摘要。
- 不登录外部系统、不发送邮件、不代表用户同意声明、不提交稿件；任何此类动作都在本 Skill 范围外。
- 安全限制 `no-measurement-invention` 发生冲突时，以更严格边界为准，并说明为什么不能继续。
...
- 若 `research_question` 缺失或无法定位：不补造内容，`decision` 为 `blocked`，并把最小补充要求写入 `operationalization_record`。
- 若 `research_question` 已定位但 `observable-terms` 尚未满足：产物只保留为 `draft` / `requires_confirmation`，不能将其作为下游已确认依据。
- 只有输入可追溯、闸门结果可说明且边界已写明时，才把 `operationalization_record` 交给下游 Skill 继续处理。

## 本单元完成前自检

- `research-question-operationalizer` 是否仅处理已声明的输入，并让 `operationalization_record` 可以回到具体的输入定位？
- `observable-terms` 是否有明确的 `pass`、`requires_confirmation` 或 `blocked` 结果，而不是被隐含跳过？
