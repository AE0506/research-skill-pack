...
description: 检查数据失败、回收不足或导师变更时的备选路径与恢复成本。 用于 Research Skill Pack 的细粒度科研工作单元；仅在用户显式调用时使用。
workflow_depth: operational
---

# 韧性审稿

检查数据失败、回收不足或导师变更时的备选路径与恢复成本。
...
- 输出：Resilience Evidence Card 与 Plan B。。按项目契约创建新版本化 artifact，包含依赖、变更理由和三项证据字段；绝不静默覆盖。
...
- **本单元：**`resilience-reviewer`，属于 Literature, Gap & Review Board。只在它自己的输入已出现、需要产生 `resilience_review_card` 或需要检查 `fallback-defined` 时调用。
- **Catalog 输入：**`fallback_options`、`timeline_baseline`。先记录每项是用户提供、已有 artifact、可定位本地资料、未知，还是仅为模型推断。
- **Catalog 输出：**`resilience_review_card`。输出是受限建议、草稿或版本化记录，不自动改变研究状态。
- **本单元闸门：**`fallback-defined`。闸门不满足时不以“合理猜测”替代缺失信息。
- **本单元安全约束：**`advisory-only`。这些限制与项目的证据字段、版本规则同等优先。

## 前置核对

1. 读取 `<project>/.research/project.yaml`、关联 artifact 的版本与状态；已确认内容复用，不重复要求研究者提供。
2. 对 `fallback_options`、`timeline_baseline` 建立输入清单：来源定位、可用范围、`provenance`、`verification_status` 与缺失值必须分开。
3. 检查 `resilience-reviewer` 是否与当前研究状态相容，并逐项核对 `fallback-defined`；冲突或未知项先进入待解决清单。
4. 明确本次处理的最小对象与输出边界：只服务 `resilience_review_card`，不顺带替代上游决策、真实数据处理或外部操作。
...
4. 围绕 `resilience_review_card` 形成可追溯草稿：每个关键判断注明输入 ID、依据、限制和需要人工确认的内容。
5. 维护支持、冲突、替代和未决关系；不能因多数资料同向就抹掉反例或范围差异。
6. 把空白、问题或评审建议写成带证据边界的草稿，缺少可核对材料时回到导入、阅读或核验。

## 产物与记录

对 `resilience_review_card` 创建新版本，而不是覆盖旧记录。最小工作草图如下；字段值必须来自本次可定位输入：

```yaml
resilience_review_card:
  artifact_id: <resilience-reviewer-sequence>
  status: draft
  depends_on: [<confirmed-or-versioned-input-artifact-id>]
  input_trace:
    fallback_options: supplied | artifact_linked | missing | needs_confirmation
    timeline_baseline: supplied | artifact_linked | missing | needs_confirmation
  gate_result:
    fallback-defined: pass | unmet | not_applicable
...
- 若 `resilience_review_card` 会影响下游内容，写明 `depends_on`、`supersedes` 与受影响项；只生成建议时标记 `advisory_only`。
...
|---|---|---|---|
| `ready` | 输入可定位，`fallback-defined` 均满足，且输出边界已说明 | 写入版本化 `resilience_review_card` 草稿/记录 | 交给状态机允许的下游 Skill；是否确认仍由用户决定 |
...
- `resilience-reviewer` 只基于可定位的本地资料工作；不能联网补检索、抓取全文、伪造来源、页码、数据、结果或人工核验。
...
- 若 `fallback_options` 缺失或无法定位：不补造内容，`decision` 为 `blocked`，并把最小补充要求写入 `resilience_review_card`。
- 若 `fallback_options` 已定位但 `fallback-defined` 尚未满足：产物只保留为 `draft` / `requires_confirmation`，不能将其作为下游已确认依据。
- 只有输入可追溯、闸门结果可说明且边界已写明时，才把 `resilience_review_card` 交给下游 Skill 继续处理。

## 本单元完成前自检

- `resilience-reviewer` 是否仅处理已声明的输入，并让 `resilience_review_card` 可以回到具体的输入定位？
- `fallback-defined` 是否有明确的 `pass`、`requires_confirmation` 或 `blocked` 结果，而不是被隐含跳过？
