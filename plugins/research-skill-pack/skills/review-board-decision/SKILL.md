...
- **本单元：**`review-board-decision`，属于 Literature, Gap & Review Board。只在它自己的输入已出现、需要产生 `board_decision` 或需要检查 `fatal-flaw-screen` 时调用。
- **Catalog 输入：**`review_cards`。先记录每项是用户提供、已有 artifact、可定位本地资料、未知，还是仅为模型推断。
- **Catalog 输出：**`board_decision`。输出是受限建议、草稿或版本化记录，不自动改变研究状态。
- **本单元闸门：**`fatal-flaw-screen`。闸门不满足时不以“合理猜测”替代缺失信息。
- **本单元安全约束：**`user-confirmation-required`。这些限制与项目的证据字段、版本规则同等优先。

## 前置核对

1. 读取 `<project>/.research/project.yaml`、关联 artifact 的版本与状态；已确认内容复用，不重复要求研究者提供。
2. 对 `review_cards` 建立输入清单：来源定位、可用范围、`provenance`、`verification_status` 与缺失值必须分开。
3. 检查 `review-board-decision` 是否与当前研究状态相容，并逐项核对 `fatal-flaw-screen`；冲突或未知项先进入待解决清单。
...
```yaml
board_decision:
  artifact_id: <review-board-decision-sequence>
  status: draft
  depends_on: [<confirmed-or-versioned-input-artifact-id>]
  input_trace:
    review_cards: supplied | artifact_linked | missing | needs_confirmation
  gate_result:
    fatal-flaw-screen: pass | unmet | not_applicable
...
|---|---|---|---|
| `ready` | 输入可定位，`fatal-flaw-screen` 均满足，且输出边界已说明 | 写入版本化 `board_decision` 草稿/记录 | 交给状态机允许的下游 Skill；是否确认仍由用户决定 |
...
- `review-board-decision` 只基于可定位的本地资料工作；不能联网补检索、抓取全文、伪造来源、页码、数据、结果或人工核验。
- 原始敏感数据默认不读入模型；如涉及数据，只登记路径、哈希、权限和经批准的最小脱敏摘要。
- 不登录外部系统、不发送邮件、不代表用户同意声明、不提交稿件；任何此类动作都在本 Skill 范围外。
- 安全限制 `user-confirmation-required` 发生冲突时，以更严格边界为准，并说明为什么不能继续。
...
- 若 `review_cards` 缺失或无法定位：不补造内容，`decision` 为 `blocked`，并把最小补充要求写入 `board_decision`。
- 若 `review_cards` 已定位但 `fatal-flaw-screen` 尚未满足：产物只保留为 `draft` / `requires_confirmation`，不能将其作为下游已确认依据。
- 只有输入可追溯、闸门结果可说明且边界已写明时，才把 `board_decision` 交给下游 Skill 继续处理。

## 本单元完成前自检

- `review-board-decision` 是否仅处理已声明的输入，并让 `board_decision` 可以回到具体的输入定位？
- `fatal-flaw-screen` 是否有明确的 `pass`、`requires_confirmation` 或 `blocked` 结果，而不是被隐含跳过？
- 本次记录是否保留了证据限制、未决项与下游影响，且没有把草稿或模型推断升级为事实？

## 规范化写入

- **策略：**`canonical_writer`。本 Skill 的唯一 canonical 权限来自 [规范化写入策略表](../../shared/canonical-mutation-policy-v0.2.yaml)：artifact 为 `board_decision`；状态迁移为 `gap_ready → board_decided`, `gap_ready → PIVOT`, `gap_ready → KILL`, `gap_ready → blocked`。不按名称猜测或扩大权限。
- **开始前：**先调用 `inspect_research_project`，读取 `mutation_revision`、现有 artifact、状态闸门与未收敛收据；若状态为 `blocked`，先说明恢复条件。
- **预检：**构造一次变更请求（`request_id`、`expected_mutation_revision`、`origin_skill_id: review-board-decision`、artifact 和/或状态迁移原因），调用 `validate_canonical_change`。预检失败时不写 `.research/`。
