...
description: Simulate statistical review using registered analysis runs, assumptions,
  and results. Explicit only.
...
- 需要：manuscript、analysis_run。
- 仅接收已获授权、已脱敏或可公开使用的材料；来源状态必须可说明。

## 执行步骤

1. 先检查门槛：real-analysis-record。
...
- 产物：`statistics_review`。
- 能力说明：Simulate statistical review using registered analysis runs, assumptions, and results. Explicit only.
...
- **本单元：**`statistics-reviewer`，属于 Manuscript, Audit & Reviewer。只在它自己的输入已出现、需要产生 `statistics_review` 或需要检查 `real-analysis-record` 时调用。
- **Catalog 输入：**`manuscript`、`analysis_run`。先记录每项是用户提供、已有 artifact、可定位本地资料、未知，还是仅为模型推断。
- **Catalog 输出：**`statistics_review`。输出是受限建议、草稿或版本化记录，不自动改变研究状态。
- **本单元闸门：**`real-analysis-record`。闸门不满足时不以“合理猜测”替代缺失信息。
...
2. 对 `manuscript`、`analysis_run` 建立输入清单：来源定位、可用范围、`provenance`、`verification_status` 与缺失值必须分开。
3. 检查 `statistics-reviewer` 是否与当前研究状态相容，并逐项核对 `real-analysis-record`；冲突或未知项先进入待解决清单。
4. 明确本次处理的最小对象与输出边界：只服务 `statistics_review`，不顺带替代上游决策、真实数据处理或外部操作。
...
4. 围绕 `statistics_review` 形成可追溯草稿：每个关键判断注明输入 ID、依据、限制和需要人工确认的内容。
5. 区分可起草的研究表达、待人工补证内容与必须阻断的虚构/过度解释，不把流畅文字当作证据。
6. 保存段落、审计意见或返修任务的版本链，并把需要复核的下游章节显式交接。

## 产物与记录

对 `statistics_review` 创建新版本，而不是覆盖旧记录。最小工作草图如下；字段值必须来自本次可定位输入：

```yaml
statistics_review:
  artifact_id: <statistics-reviewer-sequence>
  status: draft
  ...
  gate_result:
    real-analysis-record: pass | unmet | not_applicable
...
- 若 `statistics_review` 会影响下游内容，写明 `depends_on`、`supersedes` 与受影响项；只生成建议时标记 `advisory_only`。
...
|---|---|---|---|
| `ready` | 输入可定位，`real-analysis-record` 均满足，且输出边界已说明 | 写入版本化 `statistics_review` 草稿/记录 | 交给状态机允许的下游 Skill；是否确认仍由用户决定 |
...
- `statistics-reviewer` 只基于可定位的本地资料工作；不能联网补检索、抓取全文、伪造来源、页码、数据、结果或人工核验。
...
- 若 `manuscript` 缺失或无法定位：不补造内容，`decision` 为 `blocked`，并把最小补充要求写入 `statistics_review`。
- 若 `manuscript` 已定位但 `real-analysis-record` 尚未满足：产物只保留为 `draft` / `requires_confirmation`，不能将其作为下游已确认依据。
- 只有输入可追溯、闸门结果可说明且边界已写明时，才把 `statistics_review` 交给下游 Skill 继续处理。

## 本单元完成前自检

- `statistics-reviewer` 是否仅处理已声明的输入，并让 `statistics_review` 可以回到具体的输入定位？
- `real-analysis-record` 是否有明确的 `pass`、`requires_confirmation` 或 `blocked` 结果，而不是被隐含跳过？
