...
- **本单元：**`submission-package-builder`，属于 Venue, Submission & Archive。只在它自己的输入已出现、需要产生 `submission_package_draft` 或需要检查 `pre-submission-checklist` 时调用。
- **Catalog 输入：**`manuscript`、`venue_assessment`。先记录每项是用户提供、已有 artifact、可定位本地资料、未知，还是仅为模型推断。
- **Catalog 输出：**`submission_package_draft`。输出是受限建议、草稿或版本化记录，不自动改变研究状态。
- **本单元闸门：**`pre-submission-checklist`。闸门不满足时不以“合理猜测”替代缺失信息。
- **本单元安全约束：**`draft-only`。这些限制与项目的证据字段、版本规则同等优先。

## 前置核对

1. 读取 `<project>/.research/project.yaml`、关联 artifact 的版本与状态；已确认内容复用，不重复要求研究者提供。
2. 对 `manuscript`、`venue_assessment` 建立输入清单：来源定位、可用范围、`provenance`、`verification_status` 与缺失值必须分开。
3. 检查 `submission-package-builder` 是否与当前研究状态相容，并逐项核对 `pre-submission-checklist`；冲突或未知项先进入待解决清单。
4. 明确本次处理的最小对象与输出边界：只服务 `submission_package_draft`，不顺带替代上游决策、真实数据处理或外部操作。
...
4. 围绕 `submission_package_draft` 形成可追溯草稿：每个关键判断注明输入 ID、依据、限制和需要人工确认的内容。
5. 把投稿包、回复或归档记录保持为本地待确认草稿；每个改动都回链到稿件版本和证据。
6. 发现缺作者确认、伦理/版权信息、关键格式或证据问题时明确阻断，绝不登录、发送或提交。

## 产物与记录

对 `submission_package_draft` 创建新版本，而不是覆盖旧记录。最小工作草图如下；字段值必须来自本次可定位输入：

```yaml
submission_package_draft:
  artifact_id: <submission-package-builder-sequence>
  status: draft
  ...
  gate_result:
    pre-submission-checklist: pass | unmet | not_applicable
...
- 若 `submission_package_draft` 会影响下游内容，写明 `depends_on`、`supersedes` 与受影响项；只生成建议时标记 `advisory_only`。
...
|---|---|---|---|
| `ready` | 输入可定位，`pre-submission-checklist` 均满足，且输出边界已说明 | 写入版本化 `submission_package_draft` 草稿/记录 | 交给状态机允许的下游 Skill；是否确认仍由用户决定 |
...
- `submission-package-builder` 只基于可定位的本地资料工作；不能联网补检索、抓取全文、伪造来源、页码、数据、结果或人工核验。
...
- 若 `manuscript` 缺失或无法定位：不补造内容，`decision` 为 `blocked`，并把最小补充要求写入 `submission_package_draft`。
- 若 `manuscript` 已定位但 `pre-submission-checklist` 尚未满足：产物只保留为 `draft` / `requires_confirmation`，不能将其作为下游已确认依据。
- 只有输入可追溯、闸门结果可说明且边界已写明时，才把 `submission_package_draft` 交给下游 Skill 继续处理。

## 本单元完成前自检

- `submission-package-builder` 是否仅处理已声明的输入，并让 `submission_package_draft` 可以回到具体的输入定位？
- `pre-submission-checklist` 是否有明确的 `pass`、`requires_confirmation` 或 `blocked` 结果，而不是被隐含跳过？
