...
description: 以理论适用性、机制清晰度和边界条件审查候选研究问题。 用于 Research Skill Pack 的细粒度科研工作单元；仅在用户显式调用时使用。
workflow_depth: operational
---

# 理论审稿

以理论适用性、机制清晰度和边界条件审查候选研究问题。

## 前置与输入

- 读取[项目契约](../../shared/project-contract.md)与[状态机](../../shared/state-machine.md)，并检查项目状态、已有 artifact 和依赖。
- 输入：候选问题、理论地图与 Context。

## 执行与产物

- 只处理满足前置条件的材料；缺少依赖时说明缺口并路由至对应上游 Skill。
- 输出：Theory Review Evidence Card。。按项目契约创建新版本化 artifact，包含依赖、变更理由和三项证据字段；绝不静默覆盖。
...
- **本单元：**`theory-reviewer`，属于 Literature, Gap & Review Board。只在它自己的输入已出现、需要产生 `theory_review_card` 或需要检查 `theory-links` 时调用。
- **Catalog 输入：**`research_question`、`theory_map`。先记录每项是用户提供、已有 artifact、可定位本地资料、未知，还是仅为模型推断。
- **Catalog 输出：**`theory_review_card`。输出是受限建议、草稿或版本化记录，不自动改变研究状态。
...
3. 检查 `theory-reviewer` 是否与当前研究状态相容，并逐项核对 `theory-links`；冲突或未知项先进入待解决清单。
4. 明确本次处理的最小对象与输出边界：只服务 `theory_review_card`，不顺带替代上游决策、真实数据处理或外部操作。
...
4. 围绕 `theory_review_card` 形成可追溯草稿：每个关键判断注明输入 ID、依据、限制和需要人工确认的内容。
5. 维护支持、冲突、替代和未决关系；不能因多数资料同向就抹掉反例或范围差异。
6. 把空白、问题或评审建议写成带证据边界的草稿，缺少可核对材料时回到导入、阅读或核验。

## 产物与记录

对 `theory_review_card` 创建新版本，而不是覆盖旧记录。最小工作草图如下；字段值必须来自本次可定位输入：

```yaml
theory_review_card:
  artifact_id: <theory-reviewer-sequence>
  status: draft
...
- 若 `theory_review_card` 会影响下游内容，写明 `depends_on`、`supersedes` 与受影响项；只生成建议时标记 `advisory_only`。
...
|---|---|---|---|
| `ready` | 输入可定位，`theory-links` 均满足，且输出边界已说明 | 写入版本化 `theory_review_card` 草稿/记录 | 交给状态机允许的下游 Skill；是否确认仍由用户决定 |
...
- `theory-reviewer` 只基于可定位的本地资料工作；不能联网补检索、抓取全文、伪造来源、页码、数据、结果或人工核验。
...
- 若 `research_question` 缺失或无法定位：不补造内容，`decision` 为 `blocked`，并把最小补充要求写入 `theory_review_card`。
- 若 `research_question` 已定位但 `theory-links` 尚未满足：产物只保留为 `draft` / `requires_confirmation`，不能将其作为下游已确认依据。
- 只有输入可追溯、闸门结果可说明且边界已写明时，才把 `theory_review_card` 交给下游 Skill 继续处理。

## 本单元完成前自检

- `theory-reviewer` 是否仅处理已声明的输入，并让 `theory_review_card` 可以回到具体的输入定位？
